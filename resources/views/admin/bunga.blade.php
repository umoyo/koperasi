...
<div class="card shadow mb-4">
   <div class="card-header py-3">
      <div class="row">
       ...
       <div class="text-right">Saldo total: Rp.<span class=" saldototal">{{number_format( $identitas->dataSaldo->sum('saldo'),2)}}</span></div>
       <input type="hidden" id="saldoinput" value="{{$identitas->dataSaldo->sum('saldo')}}">
       </div>
      </div>
   </div>
<div class="card-body">
...
@if ($b->bulan >= $identitas->created_at->format("Y-m"))
 <tr>
 <td class="nomor"> {{$index +1}} </td>
 <td class="nomor"> {{$b->bulan}} </td>
 <td>  Rp. {{number_format($identitas->dataSaldo()->where('created_at','LIKE','%'.$b->bulan.'%')->sum('saldo'),2)}}</td>
 <td>  Rp. {{number_format($identitas->dataSaldo()->where('created_at','LIKE','%'.$b->bulan.'%')->sum('saldo') / 100 * 0.5,2)}}</td>
 <td><form method="post" id="bunga{{$b->id_enc}}" action="{{url('/bunga')}}">
    <input type="hidden" name="id_enc" value="{{$identitas->id_enc}}">
    <input type="hidden" name="bunga" value=" {{$identitas->dataSaldo()->where('created_at','LIKE','%'.$b->bulan.'%')->sum('saldo') / 100 * 0.5}}">
    @csrf
    <button form="bunga{{$b->id_enc}}" class="btn" type="submit" >Tambah Bunga</button>
    </form>
 </td>
 <!--
 <td>  {{$identitas->dataTotalSaldo()->where('created_at','LIKE','%'.$b->bulan.'%')->orderBy('saldo', 'asc')->min('saldo')}}</td>
 <td>  {{$identitas->dataTotalSaldo()->where('created_at','LIKE','%'.$b->bulan.'%')->orderBy('saldo', 'asc')->min('saldo') / 100 * 0.5}}</td>
 <td><form method="post" id="bunga{{$b->id_enc}}" action="{{url('/bunga')}}">
    <input type="hidden" name="id_enc" value="{{$identitas->id_enc}}">
    <input type="hidden" name="bunga" value=" {{$identitas->dataTotalSaldo()->where('created_at','LIKE','%'.$b->bulan.'%')->orderBy('saldo', 'asc')->min('saldo') / 100 * 0.5}}">
    @csrf
    <button form="bunga{{$b->id_enc}}" class="btn" type="submit" >Tambah Bunga</button>
    </form>
 </td>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    @csrf
                    <input type="text" class="form-control" id="id_enc" value="{{$identitas->id_enc}}">
